...
def d4_path_maker(n, a, b, c, d): #n is for number of sides of dice
   path_columns = []
   #these tuples point to the map value of the previous column, in this case, all 0 because there isn't a previous column. 
   #the second map inside the first and the second int in the tuple are how many bcd strings are downstream. The next of each is cd strings downstream.
   path_columns.append({1:{1:{1:{a + b + c + d:(0, 0, 0)}}}, 0:{1:{1:{b + a + c + d:(0, 0, 0), b + c + a + d:(0, 0, 0), b + c + d + a:(0, 0, 0)}}, 0:{1:{a + c + b + d:(0, 0, 0), a + c + d + b:(0, 0, 0), c + a + b + d:(0, 0, 0), c + a + d + b:(0, 0, 0), c + b + a + d:(0, 0, 0), c + b + d + a:(0, 0, 0), c + d + a + b:(0, 0, 0), c + d + b + a:(0, 0, 0)}, 0:{a + b + d + c:(0, 0, 0), a + d + b + c:(0, 0, 0), a + d + c + b:(0, 0, 0), b + a + d + c:(0, 0, 0), b + d + a + c:(0, 0, 0), b + d + c + a:(0, 0, 0), d + a + b + c:(0, 0, 0), d + a + c + b:(0, 0, 0), d + b + a + c:(0, 0, 0), d + b + c + a:(0, 0, 0), d + c + a + b:(0, 0, 0), d + c + b + a:(0, 0, 0)}}}})
   for i in range(n - 1):
      path_columns.append({})
      d4_path_column_maker(a + b + c + d, i + 2,  True, i + 2, i + 2, path_columns[i], path_columns[i + 1])
      d4_path_column_maker(a + b + d + c,     0,  True, i + 2, i + 1, path_columns[i], path_columns[i + 1])
      d4_path_column_maker(a + c + b + d,     0,  True,     0, i + 2, path_columns[i], path_columns[i + 1])
      d4_path_column_maker(a + c + d + b,     0,  True,     0, i + 2, path_columns[i], path_columns[i + 1])
      d4_path_column_maker(a + d + b + c,     0,  True, i + 2, i + 1, path_columns[i], path_columns[i + 1])
      d4_path_column_maker(a + d + c + b,     0,  True,     0, i + 1, path_columns[i], path_columns[i + 1])
      d4_path_column_maker(b + a + c + d, i + 2, False, i + 2, i + 2, path_columns[i], path_columns[i + 1])
      d4_path_column_maker(b + a + d + c,     0, False, i + 2, i + 1, path_columns[i], path_columns[i + 1])
      d4_path_column_maker(b + c + a + d, i + 2, False, i + 2, i + 2, path_columns[i], path_columns[i + 1])
      d4_path_column_maker(b + c + d + a, i + 2, False, i + 2, i + 2, path_columns[i], path_columns[i + 1])
      d4_path_column_maker(b + d + a + c,     0, False, i + 2, i + 1, path_columns[i], path_columns[i + 1])
      d4_path_column_maker(b + d + c + a,     0, False, i + 2, i + 1, path_columns[i], path_columns[i + 1])
      d4_path_column_maker(c + a + b + d,     0,  True,     0, i + 2, path_columns[i], path_columns[i + 1])
      d4_path_column_maker(c + a + d + b,     0,  True,     0, i + 2, path_columns[i], path_columns[i + 1])
      d4_path_column_maker(c + b + a + d,     0, False,     0, i + 2, path_columns[i], path_columns[i + 1])
      d4_path_column_maker(c + b + d + a,     0, False,     0, i + 2, path_columns[i], path_columns[i + 1])
      d4_path_column_maker(c + d + a + b,     0,  True,     0, i + 2, path_columns[i], path_columns[i + 1])
      d4_path_column_maker(c + d + b + a,     0, False,     0, i + 2, path_columns[i], path_columns[i + 1])
      d4_path_column_maker(d + a + b + c,     0,  True, i + 2, i + 1, path_columns[i], path_columns[i + 1])
      d4_path_column_maker(d + a + c + b,     0,  True,     0, i + 1, path_columns[i], path_columns[i + 1])
      d4_path_column_maker(d + b + a + c,     0, False, i + 2, i + 1, path_columns[i], path_columns[i + 1])
      d4_path_column_maker(d + b + c + a,     0, False, i + 2, i + 1, path_columns[i], path_columns[i + 1])
      d4_path_column_maker(d + c + a + b,     0,  True,     0, i + 1, path_columns[i], path_columns[i + 1])
      d4_path_column_maker(d + c + b + a,     0, False,     0, i + 1, path_columns[i], path_columns[i + 1])
   return path_columns

def d4_path_column_maker(group, num_abcs, is_ab, num_bcs, num_cs, previous_path_column, current_path_column):
   for path_abcd_num in previous_path_column:
      for path_bcd_num in previous_path_column[path_abcd_num]:
         for path_cd_num in previous_path_column[path_abcd_num][path_bcd_num]:
            if is_ab:
               total_abcd = num_abcs + path_cd_num + path_bcd_num + path_abcd_num
            else:
               total_abcd = num_abcs + path_bcd_num + path_abcd_num
            total_bcd = num_bcs + path_cd_num + path_bcd_num
            total_cd = num_cs + path_cd_num
            #if group == 'dcba':
            #   print(group)
            #   print(path_abcd_num)
            #   print(path_bcd_num)
            ...
            #   print(total_bcd)
            #   print(total_cd)
            if total_abcd not in current_path_column:
               current_path_column[total_abcd] = {}
            if total_bcd not in current_path_column[total_abcd]:
               current_path_column[total_abcd][total_bcd] = {}
            if total_cd not in current_path_column[total_abcd][total_bcd]:
               current_path_column[total_abcd][total_bcd][total_cd] = {}
            if group in current_path_column[total_abcd][total_bcd][total_cd]:
               print('uh oh')
            current_path_column[total_abcd][total_bcd][total_cd][group] = (path_abcd_num, path_bcd_num, path_cd_num)
...
def d4_path_finder(n):
   ...
   share_bcd = pow(n, 3) / math.factorial(3)
   share_cd = pow(n, 2) / math.factorial(2)
   #print(share)
   return d4_path_finder_recursive(n, share, share, share, share, share, share, share, share, share, share, share, share, share, share, share, share, share, share, share, share, share, share, share, share, share_bcd, share_bcd, share_bcd, share_bcd, share_bcd, share_bcd, share_bcd, share_bcd, share_bcd, share_bcd, share_bcd, share_bcd, share_bcd, share_bcd, share_bcd, share_bcd, share_bcd, share_bcd, share_bcd, share_bcd, share_bcd, share_bcd, share_bcd, share_bcd, share_cd, share_cd, share_cd, share_cd, share_cd, share_cd, share_cd, share_cd, share_cd, share_cd, share_cd, share_cd, abcd_path_columns, abdc_path_columns, acbd_path_columns, acdb_path_columns, adbc_path_columns, adcb_path_columns, bacd_path_columns, badc_path_columns, bcad_path_columns, bcda_path_columns, bdac_path_columns, bdca_path_columns, cabd_path_columns, cadb_path_columns, cbad_path_columns, cbda_path_columns, cdab_path_columns, cdba_path_columns, dabc_path_columns, dacb_path_columns, dbac_path_columns, dbca_path_columns, dcab_path_columns, dcba_path_columns)
...
def d4_path_finder_recursive(n, abcd_share, abdc_share, acbd_share, acdb_share, adbc_share, adcb_share, bacd_share, badc_share, bcad_share, bcda_share, bdac_share, bdca_share, cabd_share, cadb_share, cbad_share, cbda_share, cdab_share, cdba_share, dabc_share, dacb_share, dbac_share, dbca_share, dcab_share, dcba_share, abc_share, abd_share, acb_share, acd_share, adb_share, adc_share, bac_share, bad_share, bca_share, bcd_share, bda_share, bdc_share, cab_share, cad_share, cba_share, cbd_share, cda_share, cdb_share, dab_share, dac_share, dba_share, dbc_share, dca_share, dcb_share, ab_share, ac_share, ad_share, ba_share, bc_share, bd_share, ca_share, cb_share, cd_share, da_share, db_share, dc_share, abcd_path_columns, abdc_path_columns, acbd_path_columns, acdb_path_columns, adbc_path_columns, adcb_path_columns, bacd_path_columns, badc_path_columns, bcad_path_columns, bcda_path_columns, bdac_path_columns, bdca_path_columns, cabd_path_columns, cadb_path_columns, cbad_path_columns, cbda_path_columns, cdab_path_columns, cdba_path_columns, dabc_path_columns, dacb_path_columns, dbac_path_columns, dbca_path_columns, dcab_path_columns, dcba_path_columns, depth=0, so_far=''):
   solutions = []
   for abcd_possibility in abcd_path_columns[n - depth - 1][abcd_share][bcd_share][cd_share]:
      if depth == 0 and abcd_possibility != 'abcd':
         continue
      if abcd_possibility in abdc_path_columns[n - depth - 1][abdc_share][bdc_share][dc_share]:
         if abcd_possibility in acbd_path_columns[n - depth - 1][acbd_share][cbd_share][bd_share]:
            if abcd_possibility in acdb_path_columns[n - depth - 1][acdb_share][cdb_share][db_share]:
               if abcd_possibility in adbc_path_columns[n - depth - 1][adbc_share][dbc_share][bc_share]:
                  if abcd_possibility in adcb_path_columns[n - depth - 1][adcb_share][dcb_share][cb_share]:
                     if abcd_possibility in bacd_path_columns[n - depth - 1][bacd_share][acd_share][cd_share]:
                        if abcd_possibility in badc_path_columns[n - depth - 1][badc_share][adc_share][dc_share]:
                           if abcd_possibility in bcad_path_columns[n - depth - 1][bcad_share][cad_share][ad_share]:
                              if abcd_possibility in bcda_path_columns[n - depth - 1][bcda_share][cda_share][da_share]:
                                 if abcd_possibility in bdac_path_columns[n - depth - 1][bdac_share][dac_share][ac_share]:
                                    if abcd_possibility in bdca_path_columns[n - depth - 1][bdca_share][dca_share][ca_share]:
                                       if abcd_possibility in cabd_path_columns[n - depth - 1][cabd_share][abd_share][bd_share]:
                                          if abcd_possibility in cadb_path_columns[n - depth - 1][cadb_share][adb_share][db_share]:
                                             if abcd_possibility in cbad_path_columns[n - depth - 1][cbad_share][bad_share][ad_share]:
                                                if abcd_possibility in cbda_path_columns[n - depth - 1][cbda_share][bda_share][da_share]:
                                                   if abcd_possibility in cdab_path_columns[n - depth - 1][cdab_share][dab_share][ab_share]:
                                                      if abcd_possibility in cdba_path_columns[n - depth - 1][cdba_share][dba_share][ba_share]:
                                                         if abcd_possibility in dabc_path_columns[n - depth - 1][dabc_share][abc_share][bc_share]:
                                                            if abcd_possibility in dacb_path_columns[n - depth - 1][dacb_share][acb_share][cb_share]:
                                                               if abcd_possibility in dbac_path_columns[n - depth - 1][dbac_share][bac_share][ac_share]:
                                                                  if abcd_possibility in dbca_path_columns[n - depth - 1][dbca_share][bca_share][ca_share]:
                                                                     if abcd_possibility in dcab_path_columns[n - depth - 1][dcab_share][cab_share][ab_share]:
                                                                        if abcd_possibility in dcba_path_columns[n - depth - 1][dcba_share][cba_share][ba_share]:
                                                                           if depth == n - 1:
                                                                              solutions.append(so_far + abcd_possibility)
                                                                           else:
                                                                              #didn't get to changing these:
...
for path_column in d4_path_maker(12, 'a', 'b', 'c', 'd'):
   ...
   for num_abcd in path_column:
      for num_bcd in path_column[num_abcd]:
         for num_cd in path_column[num_abcd][num_bcd]:
            n += len(path_column[num_abcd][num_bcd][num_cd])
   print(n)
   #print(path_column)
   i += 1
...
print(d4_path_maker(12, 'a', 'b', 'c', 'd')[11][864][288][72])
#print(d4_path_maker(6, 'a', 'b', 'c', 'd')[5][54][36])
